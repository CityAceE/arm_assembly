...
macro	uart_mem_write16 addr, val {
	mov32	r0, addr
	mov	r1, val and 0x00ff
	add	r1, val and 0xff00
	str	r1, [r0]
}
...
uart_init:
	...
	bic	r1, r0, 0x3F000
	orr	r1, r1, 0x12000
	uart_mem_write8 PERIPHERAL_BASE + GPIO_BASE + GPIO_GPFSEL1, r1
...
;-----------------------
; Отправляет в UART1 текст, адрес которого задан в r1
uart_send_text:
	mov	r6, r1
	mov32	r2, AUX_MU_LSR_REG
	mov32	r3, AUX_MU_IO_REG
next_uart_symbol:
	ldrb	r4, [r6]
	cmp	r4, 0
	beq	uart_send_end
...
MESSAGE_CONS:
	dw	message_cons
message_cons:
	db	27, "[41m", 27, "[93m"	; Красный фон, жёлтые буквы
	db	"Hello, World!"
	db	27, "[0m"		; Сброс цвета на стандарт
	db	13, 10			; "\r\n"
	db	0			; Конец строки
